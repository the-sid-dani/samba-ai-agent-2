...
SLACK_API_CHANNELS_PER_PAGE = 50  # Reduced to avoid rate limiting
SLACK_MAX_RETURNED_CHANNELS = 200  # Reduced for better performance
...
def get_all_channels_from_slack_api(
    ...
    client = WebClient(token=tokens["bot_token"], timeout=30)  # Increased timeout
    ...
    try:
        # Start with public channels only to avoid permission issues
        channel_types = "public_channel"
        
        # Use conversations_list to get channels in the workspace
        while len(all_channels) < SLACK_MAX_RETURNED_CHANNELS and page_count < max_pages:
            page_count += 1
            ...
            if next_cursor:
                api_kwargs["cursor"] = next_cursor

            response = client.conversations_list(**api_kwargs)

            # Add channels to our list
            if "channels" in response and response["channels"]:
                all_channels.extend(response["channels"])
                logger.debug(f"Fetched {len(response['channels'])} channels (page {page_count})")
        ...
        # Truncate to limit if we got too many
        if len(all_channels) > SLACK_MAX_RETURNED_CHANNELS:
            all_channels = all_channels[:SLACK_MAX_RETURNED_CHANNELS]
        ...
        logger.error(f"Slack API error (code: {error_code}): {error_msg}")
        
        # Provide specific error messages for common issues
        if "missing_scope" in error_msg:
            detail = (
                "Bot token missing required permissions. "
                "Please ensure the bot has 'channels:read' scope."
            )
        elif "rate_limited" in error_msg or error_code == 429:
            detail = (
            ...
            detail = (
                "Invalid bot token. "
                "Please check your Slack bot configuration."
            )
        elif "account_inactive" in error_msg:
            detail = (
                "Slack workspace is inactive. "
                "Please check your Slack workspace status."
            )
        else:
            detail = f"Error fetching channels from Slack API: {error_msg}"
