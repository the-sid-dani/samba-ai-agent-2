...
def setup_postgres(db_session: Session) -> None:
    ...
    if GEN_AI_API_KEY and fetch_default_provider(db_session) is None:
        # Only for dev flows - detect provider based on model names
        llm_model = GEN_AI_MODEL_VERSION or "gpt-4o-mini"
        fast_model = FAST_GEN_AI_MODEL_VERSION or "gpt-4o-mini"
        
        # Detect provider based on model names
        if llm_model.startswith("claude") or fast_model.startswith("claude"):
            logger.notice("Setting up default Anthropic LLM for dev (Claude models detected).")
            provider_name = "litellm"  # Use litellm for Anthropic
            provider_display_name = "DevEnvPresetAnthropic"
            # Claude model names for configuration
            claude_models = [
            ...
            provider_display_name = "DevEnvPresetOpenAI"
            model_configs = [
                ModelConfigurationUpsertRequest(name=name, is_visible=True)
